...
async function refresh(){try{
 ...
 el('online').textContent=`${live.length} / 5`;el('height').textContent=first&&!first.error?first.height.toLocaleString():'—';
 el('active').textContent=first&&!first.error?`${(first.registrations||[]).filter(r=>r.active).length} / 5`:'—';
 const hashes=new Set(live.map(n=>`${n.height}:${n.hash}`));
 el('notice').textContent=live.length<5?'일부 노드가 준비 중이거나 응답하지 않습니다. 아래 관측 시각을 확인하세요.':hashes.size===1?'5개 노드의 관측 높이와 블록 해시가 일치합니다.':'노드별 높이 또는 해시가 다릅니다. 전파 중일 수 있으므로 다음 갱신을 확인하세요.';
