...
func parseNodes(records []string) []*enode.Node {
	nodes := make([]*enode.Node, 0, len(records))
	for _, record := range records {
		var node enode.Node
		if err := node.UnmarshalText([]byte(record)); err != nil {
			panic(err)
		}
