...
func New(stack *node.Node, config *ethconfig.Config) (*LightEthereum, error) {
	...
	leth := &LightEthereum{
		...
		merger:          merger,
		engine:          ethconfig.CreateConsensusEngine(stack, &config.Ethash, chainConfig.Clique, chainConfig.Eccpow, chainConfig.Kaiju, chainConfig.Eccbeta, chainConfig.Vct, nil, false, chainDb),
