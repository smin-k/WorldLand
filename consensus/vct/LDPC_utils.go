...
// MakeDecision checks if outputWord satisfies the PoW decision criterion.
...
// MakeDecision_Seoul checks if outputWord satisfies the Seoul-era PoW decision criterion.
func MakeDecision_Seoul(header *types.Header, colInRow [][]int, outputWord []int) (bool, int) {
	...
	if numOfOnes >= parameters.n/4 && numOfOnes <= parameters.n/4*3 {
		return true, numOfOnes
	}
	return false, numOfOnes
}
